...
VIEWS_FIRST_TOPIC_RULES = (
    ...
    "- Keep everything platform-safe: no explicit sexual content, no fetish framing, no nudity, and no minors.\n"
)

NICHE_ANGLE_RULES = {
    "glam": (
        "Niche-specific angle:\n"
        "- Focus on adult dance energy, cosplay reveals, outfit transitions, glam confidence, cute-but-bold styling, public reactions, and scroll-stopping presence.\n"
        "- Keep it playful, trendy, and visually magnetic without explicit sexual content, nudity, fetish framing, or anything involving minors.\n"
        "- Make the hook feel like people need to see the moment, the reaction, or the transformation.\n"
    ),
    "celebrity": (
        "Niche-specific angle:\n"
        "- Focus on public event moments, awkward interactions, image flips, glow-up arcs, rumored tension, red-carpet reactions, glam fashion moments, and fan debate.\n"
        "- Use glamorous, high-attention framing without sexual explicitness.\n"
        "- Make the payoff feel like social tea or a surprising behind-the-scenes reveal.\n"
    ),
    "reddit": (
        "Niche-specific angle:\n"
        "- Focus on short POV drama, relationship twists, public embarrassment, savage comebacks, and awkward misunderstandings.\n"
        "- Make it feel like a story someone would retell to a friend in one breath.\n"
    ),
    "psychology": (
        "Niche-specific angle:\n"
        "- Focus on attraction psychology, social signals, manipulation tells, status games, texting behavior, jealousy, and confidence mistakes.\n"
        "- Keep claims framed as observations, patterns, or discussion starters instead of medical advice.\n"
    ),
    "theory": (
        "Niche-specific angle:\n"
        "- Focus on internet rabbit holes, hidden meanings, fan theories, unexplained clips, and pattern-spotting people argue about.\n"
        "- Make it feel debatable, bingeable, and comment-worthy.\n"
    ),
    "crime": (
        "Niche-specific angle:\n"
        "- Focus on strange disappearances, interrogation tells, scam psychology, bizarre mistakes, and unsettling timelines.\n"
        "- Avoid gore and avoid glorifying violence.\n"
    ),
    "money": (
        "Niche-specific angle:\n"
        "- Focus on rich-vs-broke behavior, money leaks, luxury signaling, hidden spending traps, and social-status decisions.\n"
        "- Keep the language punchy and personal, not corporate.\n"
    ),
    "business": (
        "Niche-specific angle:\n"
        "- Focus on profitable low-competition ideas, pricing psychology, hidden margins, and social-proof tricks.\n"
        "- Make the business angle feel fast and opportunistic, not like a lecture.\n"
    ),
    "ai": (
        "Niche-specific angle:\n"
        "- Focus on secret AI workflows, faceless income systems, unfair productivity shortcuts, and tools people feel late discovering.\n"
        "- Avoid generic 'top AI tools' lists.\n"
    ),
}

BRAINROT_VIDEO_SEARCH_POOL = [
    "adult fashion model walking city",
    "luxury lifestyle woman city",
    "beach lifestyle woman",
    "travel influencer woman",
    "female fitness gym workout",
    "yoga wellness woman sunrise",
    "beauty skincare routine woman",
    "couple luxury vacation",
    "female entrepreneur office lifestyle",
    "summer vacation poolside woman",
    "cinematic slow motion woman walking",
    "dance festival crowd woman",
    "street fashion woman montage",
    "influencer daily life aesthetic",
    "luxury car woman lifestyle",
    "rich lifestyle travel montage",
    "pov girlfriend lifestyle",
]
...
def _topic_prompt(niche: str) -> str:
    ...
    prompt += NICHE_ANGLE_RULES.get(niche, "")
...
def _brainrot_video_prompt() -> str:
    recent_titles = _recently_used_titles(ContentNiche.GLAM, limit=20)
    recent_briefs = _recent_topic_briefs(ContentNiche.GLAM, limit=12)
    local_today = timezone.localtime(timezone.now()).strftime("%B %d, %Y")
    prompt = f"Create 1 original adult glam lifestyle vertical video concept for {local_today}.\n\n"
    prompt += "Rules:\n"
    prompt += VIEWS_FIRST_TOPIC_RULES
    prompt += (
        "- This mode is video-only. Build the concept around stock video clips, not still images.\n"
        "- Keep the people clearly adult. No minors, no school framing, no age ambiguity.\n"
        "- Keep it platform-safe: no nudity, no explicit sexual behavior, no fetish framing, no porn wording.\n"
        "- Lean into glamorous, attractive, stylish, luxury, travel, wellness, couple, fashion, and influencer aesthetics.\n"
        "- Make the story feel like a montage arc people would watch with music and captions.\n"
        "- Think in scenes like city walk, vacation glow-up, beach, office power moves, gym, skincare, nightlife, car, travel, poolside.\n"
        "- Let the vibe feel scroll-stopping, bold, and modern without becoming explicit.\n"
        "- Make the script long enough for roughly 70 to 180 seconds.\n"
        "- Return 10 to 14 spoken beats plus intro and CTA.\n"
        "- Visual hints and video searches must be highly concrete stock-footage search queries.\n"
        "- Prefer vertical-friendly scenes, motion, close-ups, and lifestyle transitions.\n"
        "- Suggest a free stock-music vibe, not a copyrighted song title.\n"
    ...
    prompt += "\n".join(f"  - {brief}" for brief in recent_briefs) if recent_briefs else "  - none"
    prompt += "\n"
    prompt += "- Build a beginning, middle, and payoff so the final video feels intentionally sequenced.\n"
    prompt += "- Use search terms inspired by these seed categories when useful, but still make the result unique:\n"
    prompt += "\n".join(f"  - {item}" for item in BRAINROT_VIDEO_SEARCH_POOL)
...
def build_brainrot_video_topic() -> ViralTopic:
    niche = ContentNiche.GLAM
    attempts: list[str] = []
    for attempt in range(6):
        raw_payload, provider, model = _generate_custom_topic_payload(_brainrot_video_prompt())
        payload = _validate_topic_payload(raw_payload, niche, provider, model, bullet_limit=12, minimum_bullets=8)
        first_topic = raw_payload.get("topics", [{}])[0] if isinstance(raw_payload.get("topics"), list) else {}
        video_searches = _normalize_searches(first_topic.get("video_searches", []))
        music_vibe = truncate_text(" ".join(str(first_topic.get("music_vibe", "")).split()), 120)
        story_arc = truncate_text(" ".join(str(first_topic.get("story_arc", "")).split()), 180)
        if _is_recent_topic_duplicate(niche, payload["title"], payload["intro"], payload["bullets"]):
            attempts.append(payload["title"])
            continue
        ...
        script = "\n".join(script_lines)
        scene_plan = build_scene_plan(payload["intro"], payload["bullets"], payload["cta"], visuals=payload["visuals"])
        duration_seconds = min(180, max(70, estimate_duration_seconds(script, scene_plan) + 18))
        content_signature = stable_hash([str(niche), payload["title"].strip().lower(), " ".join(script.lower().split())])
        return ViralTopic.objects.create(
            niche=niche,
            title=payload["title"],
            hook=payload["intro"],
            ...
            seo_title=f'{payload["title"]} | {getattr(settings, "CHANNEL_BRAND_NAME", "DarkBrainScroll")}',
            description=script,
            hashtags=payload["hashtags"] or ["#glam", "#lifestyle", "#shorts", "#viral"],
            source_notes=[
                f"provider:{payload['provider']}",
                f"model:{payload['model']}",
                f"generated-at:{payload['generated_at']}",
                f"estimated-duration:{duration_seconds}",
                f"content-signature:{content_signature}",
                "render-mode:brainrot-video",
                f"music-vibe:{music_vibe}",
                f"story-arc:{story_arc}",
                *[f"asset-pack:{pack}" for pack in payload["asset_packs"]],
                *[f"video-search:{search}" for search in video_searches],
            ],
            is_trending=False,
        )
    attempted_titles = ", ".join(attempts[:6]) or "none"
    raise RuntimeError(f"Brainrot generator kept returning repeated ideas. Attempts: {attempted_titles}")
